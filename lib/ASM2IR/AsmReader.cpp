...
void AsmReader::execute() const {
  ...
  ExecEng->InstallLazyFunctionCreator(
      [=](const std::string &FuncName) -> void * {
        ...
        if (FuncName == "executeSETEQ") {
          return reinterpret_cast<void *>(executeSETEQ);
        }
        if (FuncName == "executeSETLT") {
          return reinterpret_cast<void *>(executeSETLT);
        }
        if (FuncName == "executeFLUSH") {
          return reinterpret_cast<void *>(executeFLUSH);
        }
        return nullptr;
      });
...
void AsmReader::parseAsm(std::ifstream &Input) {
  ...
  FunctionCallee executeSETEQFunc =
      M->getOrInsertFunction("executeSETEQ", int32x3FuncType);

  FunctionCallee executeSETLTFunc =
      M->getOrInsertFunction("executeSETLT", int32x3FuncType);

  FunctionCallee executeFLUSHFunc =
      M->getOrInsertFunction("executeFLUSH", voidFuncType);
  ...
  while (Input >> TokenId) {
    ...
    case ISA::SETLT: {
      ...
      case ISA::XOR:
        ...
        break;
      case ISA::SETEQ:
        builder.CreateCall(executeSETEQFunc, Args);
        break;
      case ISA::SETLT:
        builder.CreateCall(executeSETLTFunc, Args);
        break;
      }
      continue;
    }
    ...
    case ISA::FLUSH: {
      builder.CreateCall(executeFLUSHFunc);
      continue;
    }
    case ISA::EXIT: {
